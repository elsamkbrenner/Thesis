...
GIFTs_functions_community_df$sample <- rownames(GIFTs_functions_community_df)

GIFTs_functions_community_df <- dplyr::left_join(GIFTs_functions_community_df, selected_columns_func, by = "sample")

Wilcox_result_functions_community <- c()
for (y in GIFTs_functions_community_df$sample){
  res.wilcox <- wilcox.test(phylum_table[,y] ~ region, data = phylum_table,
                            exact = FALSE, alternative = "less")
  Wilcox_result <- rbind(Wilcox_result,c(res.wilcox[[1]],Pvalue=res.wilcox[[3]]))
}
