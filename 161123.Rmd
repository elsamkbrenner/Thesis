...
```{r}
# Plot
ggplot(GIFTs_domains_community_df, aes(x = region, y = Biosynthesis, group = region, color = region)) +
...
GIFTs_domains_community_df %>%
  filter(region %in% c("Daneborg", "Ittoqqortoormii")) %>%
  group_by(region) %>%
  summarise(wilcox_p = wilcox.test(Biosynthesis ~ region, data = .)$p.value) -> wilcox_results
print(wilcox_results)
