...
# Tax table
GIFTs_table <- GIFTs_elements %>%
  reshape2::melt() %>%
  dplyr::rename(Genome = Var1, Code_element = Var2, GIFT = value) %>%
  inner_join(GIFT_db,by="Code_element")

GIFTs_table1 <- GIFTs_table[,-1]
GIFTsmatrix <- as.matrix(GIFTs_table1[1:8])
rownames(GIFTsmatrix) <- GIFTs_table$Genome
functable <- tax_table(GIFTsmatrix) 
functable <- functable[!duplicated_taxa, ]
...
# physeq_clr_FUNC <- microbiome::aggregate_taxa(physeq_clr_FUNC, "Code_bundle")
```
