...
# 3. Alpha diversity calculations - Community Level
...
Adonis using old ordering system
...
## results match those of EHI workflow - yay
```
...
physeq_clr <- microbiome::transform(physeq, 'clr') # clr transformation is not working for further analysis
...
# first conglomerate at phylum level
...
# DESeq runs with its own transformation, so the raw data (instead of the clr transformed data) is appropriate here. 

# run standard DESeq analysis
region_deseq_phylum <- DESeq(region_deseq_phylum)
results_table <- results(region_deseq_phylum)
```
Basic plotting of Data: 
```{r}
plotMA(region_deseq_phylum)
plotDispEsts(region_deseq_phylum, main = "Dispersion plot Greenland dogs")
```
Blue dots above x-axis indicate genes that are over-expressed (n=127) where blue dots below the x-axis are genes that are under-expressed (n= 76). X-axis is the mean of normalized counts, and the y-axis is the log fold change.
...
# [2] see 'independentFiltering' argument of ?results
```
So from this information, we know that at the there are 3 phylums that are significantly differently abundant between Ittoq and Daneborg
...
Heatmap
```{r}
# Heatmap showing top diff expressed genes

library(pheatmap)
top_phylum <- rownames(sigtab_deseq_region_phylum_with_tax)[sigtab_deseq_region_phylum_with_tax$padj < 0.005]
pheatmap(assay(region_deseq)[top_phylum, ],scale = "row", cluster_cols = F, labels_col = all_metadata$region, labels_row = sigtab_deseq_region_phylum_with_tax$phylum, main = "Significantly Differentially Expressed Genes at the Phylum level")
```


### Genus level
```{r}
# first conglomerate at genus level
...
Basic plotting of Data: 
```{r}
plotMA(region_deseq_genus)
plotDispEsts(region_deseq_genus, main = "Dispersion plot Greenland dogs")
```
Blue dots above x-axis indicate genes that are over-expressed (n=127) where blue dots below the x-axis are genes that are under-expressed (n= 76). X-axis is the mean of normalized counts, and the y-axis is the log fold change.
...
# [2] see 'independentFiltering' argument of ?results
```
So from this information, we know that at the there are 19 genusus that are significantly differently abundant between Ittoq and Daneborg
```{r}
...
```{r}
physeq.phylum <- tax_glom(physeq, 'phylum')
tse = mia::makeTreeSummarizedExperimentFromPhyloseq(physeq.phylum)
...
## Making phyloseq object
# Coerce it into a phylseq
FuncOTU <- otu_table(count_filtered_use_this, taxa_are_rows = T)
...
## Merge into a phyloseq object
physeq_FUNC <- phyloseq(FuncOTU, treephylo, sample_tab,taxtable)
...
table.rel_ittoq_func <- physeq_func_ittoq@otu_table
means.table.rel1_ittoq_func <- as.data.frame(rowMeans(table.rel_ittoq_func))
sd.table.rel1_ittoq_func <- as.data.frame(matrixStats::rowSds(table.rel_ittoq_func, useNames = TRUE))
summary.func_ittoq <- merge(means.table.rel1_ittoq_func, sd.table.rel1_ittoq_func, by="row.names")
colnames(summary.func_ittoq) <- c("Gene","Mean", "SD")
...
table.rel_daneborg_func <- physeq_func_daneborg@otu_table
means.table.rel1_daneborg_func <- as.data.frame(rowMeans(table.rel_daneborg_func))
sd.table.rel1_daneborg_func <- as.data.frame(matrixStats::rowSds(table.rel_daneborg_func, useNames = TRUE))
summary.func_daneborg <- merge(means.table.rel1_daneborg_func, sd.table.rel1_daneborg_func, by="row.names")
colnames(summary.func_daneborg) <- c("Gene","Mean", "SD")
...
sample_type_func <- all_metadata[,c(1,4)]
...
table.W_func <- tibble::rownames_to_column(physeq_clr_func.t, "sample")
sample_type <- metadata[,c(1,4)]
func_table <- merge(table.W,sample_type,by="sample")
func_table <- func_table[,-c(1)]
func_table.no <- func_table[,-ncol(phylum_table)]
all.func <- colnames(func_table.no)
Wilcox_result_func <- c()

func_table[, y] <- as.numeric(func_table[, y])

for (y in all.func){
  res.wilcox <- wilcox.test(func_table[,y] ~ region, data = func_table,
                            exact = FALSE, alternative = "less")
  Wilcox_result_func <- rbind(Wilcox_result,c(res.wilcox[[1]],Pvalue=res.wilcox[[3]]))
}
rownames(Wilcox_result_func) <- all.taxa
...
knitr::kable(Wilcox_result_sign_func, format = "html", full_width = F,col.names = c("Function", "W", "p-value","p-value adjusted"), digits = 10) %>%
  kable_styling(latex_options="scale_down")
```