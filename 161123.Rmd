...
# Select columns and perform operations
comp_cont <- mutate(
...
library(pheatmap)
top_genus <- rownames(sigtab_deseq_region_phylum_with_tax)[sigtab_deseq_region_phylum_with_tax$padj < 0.005]
...
```{r}
library(mia)
tse_LinDA <- mia::subsetByPrevalentTaxa(tse, detection = 0, prevalence = 0.1)
```
```{r}
...
```
So from this information, we know that at the there are 5 phyla that are significantly differently abundant between Ittoq and Daneborg according to LinDA
```{r}
tax <- data.frame(physeq@tax_table) %>%
  rownames_to_column(., "taxon")
...
# Func table (Tax table replacement)
functable <- GIFTs_elements
...
# agregate results at the code element level
# physeq_clr_FUNC <- microbiome::aggregate_taxa(physeq_clr_FUNC, "Function")
```
...
```{r}
plotMA(deseq_func)
plotDispEsts(deseq_func, main = "Dispersion plot Greenland dogs")
```
Blue dots above x-axis indicate genes that are over-expressed (n=127) where blue dots below the x-axis are genes that are under-expressed (n= 76). X-axis is the mean of normalized counts, and the y-axis is the log fold change.
...
func_matrix <- as.character(as(tax_table(functable)[row.names(sigtab_deseq_FUNC), ], "matrix"))
