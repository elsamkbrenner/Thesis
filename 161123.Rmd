...
# [2] see 'independentFiltering' argument of ?results
```
22 genusus w/ significant DA between regions
...
ancombc2_out <- ancombc2(physeq.phylum, 
                  assay_name = "counts", 
                  tax_level = NULL,
                  fix_formula = "region",
                  p_adj_method = "holm", 
...
MAGotu %>% 
    as.data.frame() %>%
    dplyr::mutate(average = rowMeans(across(everything())))  %>%
    rownames_to_column(var="genome") %>% 
    dplyr::right_join(ancom_result, by=join_by(genome==genome)) %>% 
    dplyr::mutate(significance = ifelse(p_regionIttoqqortoormii < 0.01, "1", "0")) %>%
    ggplot(., aes(x=average, y=lfc_regionIttoqqortoormii, color=significance)) +
      geom_point() +
      scale_color_manual(values = c("#cccccc","#00FFFF")) +
...
```{r}
colors_alphabetic <- ehi_phylum_colors1 %>%
  dplyr::right_join(ancom_result, by=join_by(phylum == phylum)) %>%
  select(phylum, colors) %>%
	unique() %>%
	dplyr::arrange(phylum)

ancom_result %>%
    filter(p_regionIttoqqortoormii < 0.01) %>%
    ggplot(., aes(x=lfc_regionIttoqqortoormii, y=forcats::fct_rev(phylum), color=phylum)) +
      ...
      geom_text(aes(-max(ancom_result$lfc_regionIttoqqortoormii)+1, 1), label = "Enriched in Daneborg", color="#666666") +
      geom_text(aes(max(ancom_result$lfc_regionIttoqqortoormii)-1, 1), label = "Enriched in Ittoq", color="#666666") +
      theme(legend.position='none',
          panel.background = element_blank(),
          axis.line = element_line(size = 0.5, linetype = "solid", colour = "black"),
...
ancom_results <- ancom_result %>%
  filter(lfc_regionIttoqqortoormii < 0) %>% #
  filter(q_regionIttoqqortoormii < 0.05) %>%
  arrange(lfc_regionIttoqqortoormii) %>%
  dplyr::select(genome,lfc_regionIttoqqortoormii,q_regionIttoqqortoormii, phylum)

kable(ancom_results)
```
```{r ANCOM-BC DA P plotting, comment="", echo=FALSE, message=FALSE, warning=FALSE}
ancom_results %>%
  ggplot(., aes(x=forcats::fct_rev(phylum), y=lfc_regionIttoqqortoormii, color=phylum)) + 
  geom_point(size=4) + 
  scale_color_manual(values=colors_alphabetic$colors) + 
  geom_hline(yintercept=0) + 
  coord_flip()+
...
set.seed(1234) #set seed for reproducibility
ancombc2_out_genus <- ancombc2(physeq.genus, 
                  assay_name = "counts", 
                  tax_level = NULL,
                  fix_formula = "region",
                  p_adj_method = "holm", 
...
ancom_result_genus <- ancombc2_out_genus$res %>%
    rename(genome=taxon)  %>%
    dplyr::left_join(taxonomyclean,by=join_by(genome==genome))
...
MAGotu %>% 
    ...
    dplyr::mutate(average = rowMeans(across(everything())))  %>%
    rownames_to_column(var="genome") %>% 
    dplyr::right_join(ancom_result_genus, by=join_by(genome==genome)) %>% 
    dplyr::mutate(significance = ifelse(p_regionIttoqqortoormii < 0.01, "1", "0")) %>%
    ggplot(., aes(x=average, y=lfc_regionIttoqqortoormii, color=significance)) +
      geom_point() +
      scale_color_manual(values = c("#cccccc","#00FFFF")) +
...
```{r}
colors_alphabetic <- ehi_phylum_colors1 %>%
  dplyr::right_join(ancom_result_genus, by=join_by(phylum == phylum)) %>%
  select(phylum, colors) %>%
	unique() %>%
	dplyr::arrange(phylum)
...
ancom_result_genus %>%
    ...
    ggplot(., aes(x=lfc_regionIttoqqortoormii, y=forcats::fct_rev(phylum), color=phylum)) +
      ...
      scale_color_manual(values=colors_alphabetic$colors) +
      geom_text(aes(-max(ancom_result_genus$lfc_regionIttoqqortoormii)+1, 1), label = "Enriched in Daneborg", color="#666666") +
      geom_text(aes(max(ancom_result_genus$lfc_regionIttoqqortoormii)-1, 1), label = "Enriched in Ittoq", color="#666666") +
     theme(legend.position='none',
          panel.background = element_blank(),
          axis.line = element_line(size = 0.5, linetype = "solid", colour = "black"),
          axis.title.x=element_blank())+
...
ancom_result_genus <- ancom_result_genus %>%
  filter(lfc_regionIttoqqortoormii < 0) %>%
  filter(q_regionIttoqqortoormii < 0.05) %>%
  arrange(lfc_regionIttoqqortoormii) %>%
  dplyr::select(genome, lfc_regionIttoqqortoormii, q_regionIttoqqortoormii, phylum, genus)

kable(ancom_result_genus)
```
...
ggplot(ancom_result_genus, aes(x=forcats::fct_rev(genus), y=lfc_regionIttoqqortoormii, color=phylum)) + 
  geom_point(size=4) + 
  scale_color_manual(values=colors_alphabetic$colors) + 
  geom_hline(yintercept=0) + 
  coord_flip()+
...
count_ALDEx <- count_table_cov_size_DA %>%
  as.data.frame() %>%
  mutate_all(~ . * 1e6) %>% #multiple by a million
  round(0) #round to integer

count_ALDEx.clr <- aldex.clr(count_ALDEx, 
               all_metadata$region, 
               mc.samples=128, 
               denom="all", 
               verbose=F)
...
```
```{r}
count_ALDEx.ttest <- aldex.ttest(count_ALDEx.clr, 
                hist.plot=F, 
                paired.test=F, 
                verbose=F)

count_ALDEx.effect <- aldex.effect(count_ALDEx.clr, 
              CI=T, 
              verbose=F, 
...
count_ALDEx.all <- data.frame(count_ALDEx.ttest,count_ALDEx.effect) %>%
    rownames_to_column(var="genome") %>%
    left_join(taxonomyclean,by=join_by(genome==genome))
```
Bland-Altman plot
```{r}
count_ALDEx.all %>%
...
count_ALDEx.all %>%
  ...
  arrange(effect) %>%
  dplyr::select(genome,effect,phylum, genus) %>%
  kable()
```
Top enriched in Daneborg
```{r}
count_ALDEx.all %>%
  filter(effect > 1) %>%
  filter(overlap < 0.1) %>% # >90% of the estimated effect sizes are >0
...
shapiro_results_Euk <- all_metadata %>%
  group_by(region) %>%
  summarise(shapiro_p_value = shapiro.test(Eukaryota_.)$p.value)
# Results: p = 0.00615 Daneborg, p = 0.0369 Ittoq
# THEREFORE neither region has normally distributed values for % Eukaryotic reads

# Wilcoxon Exact rank test
library(exactRankTests)
wilcox_exact_results_Euk <- exactRankTests::wilcox.exact(Eukaryota_. ~ region, data = all_metadata)
print(wilcox_exact_results_Euk)
# Results: p = 0.6329
# THEREFORE no sig difference in % Eukaryotic reads between regions
...
# Range 0.00300 - 0.02000
# Med = 0.00600, Mean = 0.00669

# Shapiro-Wilk test for normality (grouped by region)
shapiro_results_Crypto_R <- all_metadata %>%
  group_by(region) %>%
  summarise(shapiro_p_value = shapiro.test(Cryptosporidium_.)$p.value)
# Results: p = 0.0631 Daneborg, p = 0.0000802 Ittoq
# THEREFORE cannot assume normality for whole dataset
shapiro_results_Crypto_PR <- all_metadata %>%
  filter(PR_cryptosporidium != "excluded") %>%
  group_by(PR_cryptosporidium) %>%
  summarise(shapiro_p_value = shapiro.test(Cryptosporidium_.)$p.value)
# Results: p =  0.00000147 for "no" , p = 0.621 for "yes"

# Wilcoxon Exact rank test
library(exactRankTests)
wilcox_exact_results_Crypto_R <- exactRankTests::wilcox.exact(Cryptosporidium_. ~ region, data = all_metadata)
print(wilcox_exact_results_Crypto_R)
# Results: p = 0.007237
# THEREFORE there IS a significant difference in Crypto between regions
wilcox_results_Crypto_PR <- all_metadata %>%
  filter(PR_cryptosporidium != "excluded") %>%
  group_by(PR_cryptosporidium) %>%
  summarise(wilcox_p_value = wilcox.test(Cryptosporidium_. ~ PR_cryptosporidium, data = .)$p.value)
# Results: p =  0.483
# THEREFORE no significant difference detected in Cryptosporidium values, when grouped by PR results 
...
# Range 0 - 0.004
# Med = 0.0002, Mean = 0.0004107
summary((subset(all_metadata, Giardia_. != 0)$Giardia_.))

# Shapiro-Wilk test for normality (grouped by region)
shapiro_results_Giardia_R <- all_metadata %>%
  group_by(region) %>%
  summarise(shapiro_p_value = shapiro.test(Giardia_.)$p.value)
# Results: p = 6.32e-10 Daneborg, p = 4.12e- 5 Ittoq
shapiro_results_Giardia_PR <- all_metadata %>%
  filter(PR_giardia != "excluded") %>%
  group_by(PR_giardia) %>%
  summarise(shapiro_p_value = shapiro.test(Giardia_.)$p.value)
# Results: p =  1.52e-11 for "no" , p = 6.72e- 1 for "yes"
# THEREFORE cannot assume normality for whole dataset

# Wilcoxon Exact rank test
library(exactRankTests)
wilcox_exact_results_Giardia_R <- exactRankTests::wilcox.exact(Giardia_. ~ region, data = all_metadata)
print(wilcox_exact_results_Giardia_R)
# Results: p = p-value = 0.044
# THEREFORE there IS a significant difference in Giardia between regions
wilcox_results_Giardia_PR <- all_metadata %>%
  filter(PR_giardia != "excluded") %>%
  group_by(PR_giardia) %>%
  summarise(wilcox_p_value = wilcox.test(Giardia_. ~ PR_giardia, data = .)$p.value)
# Results: p =  0.514
# THEREFORE no significant difference detected in Giardia values, when grouped by PR results 

# Plot by region
ggplot(all_metadata, aes(x = region, y = Giardia_., group = region, color = region)) +
  geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
  theme_minimal() +
  theme(
...
# Plot by PR results
ggplot(all_metadata %>% filter(PR_giardia != "excluded"),
...
# Shapiro-Wilk test for normality (grouped by region)
shapiro_results_Sarcocystis_R <- all_metadata %>%
  group_by(region) %>%
  summarise(shapiro_p_value = shapiro.test(Sarcocystis_.)$p.value)
# Results: p = 1.31e-11 Daneborg, p = 2.43e-11 Ittoq
# THEREFORE cannot assume normality for whole dataset

# Wilcoxon Exact rank test
library(exactRankTests)
wilcox_exact_results_Sarcocystis_R <- exactRankTests::wilcox.exact(Sarcocystis_. ~ region, data = all_metadata)
print(wilcox_exact_results_Sarcocystis_R)
# Results: p = p-value =  0.4912
# THEREFORE there IS a NOT a sig difference in Sarcocystis between regions
wilcox_results_Sarcocystis_PR <- all_metadata %>%
  filter(PR_sarcocystis != "excluded") %>%
  group_by(PR_sarcocystis) %>%
  summarise(wilcox_p_value = wilcox.test(Sarcocystis_. ~ PR_sarcocystis, data = .)$p.value)
# Results: p =   0.706
# THEREFORE no significant difference detected in Sarcocystis values, when grouped by PR results 

# Plot by region
ggplot(all_metadata, aes(x = region, y = Sarcocystis_., group = region, color = region)) +
  geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
  theme_minimal() +
  theme(
...
# Plot by PR results
ggplot(all_metadata %>% filter(PR_sarcocystis != "excluded"),
       aes(x = PR_sarcocystis, y = Sarcocystis_., group = PR_sarcocystis, color = region)) +
  geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
  theme_minimal() +
  theme(
...
# Range 0.001 - 0.03
# Med = 0.004, Mean = 0.004897 

# Shapiro-Wilk test for normality (grouped by region)
shapiro_results_Toxoplasma_gondii_R <- all_metadata %>%
  group_by(region) %>%
  summarise(shapiro_p_value = shapiro.test(Toxoplasma_gondii_.)$p.value)
# Results: p = 0.00431 Daneborg, p = 0.00000000427 Ittoq
# THEREFORE cannot assume normality for whole dataset
...
# Results: p-value = 0.6477
# THEREFORE there IS NO significant difference in Toxoplasma_gondii between regions
wilcox_results_Toxoplasma_gondii_PR <- all_metadata %>%
  filter(sero_toxoplasma_gondii_PR != "excluded") %>%
  group_by(sero_toxoplasma_gondii_PR) %>%
  summarise(wilcox_p_value = wilcox.test(Giardia_. ~ sero_toxoplasma_gondii_PR, data = .)$p.value)
# Results:p-value = 1
...
# Plot by PR results
ggplot(all_metadata %>% filter( sero_toxoplasma_gondii_PR!= "excluded"),
       aes(x = sero_toxoplasma_gondii_PR, y = Giardia_., group = sero_toxoplasma_gondii_PR, color = region)) +
  geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
  theme_minimal() +
  theme(
...
# Range 0.005 - 0.3
# Med = 0.01, Mean = 0.01429

# Shapiro-Wilk test for normality (grouped by region)
shapiro_results_Trichinella_R <- all_metadata %>%
  group_by(region) %>%
  summarise(shapiro_p_value = shapiro.test(Trichinella_.)$p.value)
# Results: p = 2.12e-11 Daneborg, p = 9.37e- 4 Ittoq
shapiro_results_Trichinella_PR <- all_metadata %>%
  filter(sero_trichincella_spp_PR != "excluded") %>%
  group_by(sero_trichincella_spp_PR) %>%
  summarise(shapiro_p_value = shapiro.test(Trichinella_.)$p.value)
# Results: p =  0.314 for "yes"
# THEREFORE cannot assume normality for whole dataset

# Wilcoxon Exact rank test
wilcox_exact_results_Trichinella_R <- exactRankTests::wilcox.exact(Trichinella_. ~ region, data = all_metadata)
print(wilcox_exact_results_Trichinella_R)
# Results: p = 0.0002889
# THEREFORE there IS a significant difference in Trichinella between regions
...
# Plot by PR results
ggplot(all_metadata %>% filter(sero_trichincella_spp_PR != "excluded"),
       aes(x = sero_trichincella_spp_PR, y = Trichinella_., group = sero_trichincella_spp_PR, color = region)) +
  geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
  theme_minimal() +
  theme(
...
shapiro_results_Taenia_R <- all_metadata %>%
  group_by(region) %>%
  summarise(shapiro_p_value = shapiro.test(Taenia_.)$p.value)
# Results: p = 1.03e- 9 Daneborg, p = 1.45e-11 Ittoq
# THEREFORE cannot assume normality for whole dataset

# Wilcoxon Exact rank test
wilcox_exact_results_Taenia_R <- exactRankTests::wilcox.exact(Taenia_. ~ region, data = all_metadata)
print(wilcox_exact_results_Taenia_R)
# Results: p 7.409e-06
# THEREFORE there IS a significant difference in Taenia between regions
wilcox_results_Taenia_PR <- all_metadata %>%
  filter(PR_Tania_serialis != "excluded") %>%
  group_by(PR_Tania_serialis) %>%
  summarise(wilcox_p_value = wilcox.test(Taenia_. ~ PR_Tania_serialis, data = .)$p.value)
# Results: p =  0.0732
# THEREFORE no significant difference detected in Taenia values, when grouped by PR results 

# Plot by region
ggplot(all_metadata, aes(x = region, y = Taenia_., group = region, color = region)) +
  geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
  theme_minimal() +
  theme(
...
# Plot by PR results
ggplot(all_metadata %>% filter(PR_Tania_serialis != "excluded"),
       aes(x = PR_Tania_serialis, y = Taenia_., group = PR_Tania_serialis, color = region)) +
  geom_boxplot(alpha = 0.2, outlier.shape = NA, width = 0.3, show.legend = FALSE) +
  geom_jitter(width = 0.1, show.legend = TRUE) +
  theme_minimal() +
  theme(
...
shapiro_results_Uncinaria <- all_metadata %>%
  group_by(region) %>%
  summarise(shapiro_p_value = shapiro.test(Uncinaria_.)$p.value)
# Results: p = 1.71e-11 Daneborg, p =  3.81e-10 Ittoq
# THEREFORE neither region has normally distributed values

# Wilcoxon Exact rank test
wilcox_exact_results_Uncinaria <- exactRankTests::wilcox.exact(Uncinaria_. ~ region, data = all_metadata)
print(wilcox_exact_results_Uncinaria)
# Results: p =0.0001526
# THEREFORE YES sig difference between regions
...
summary(all_metadata$Echinococcus_.)
# Range 0.003 - 0.08
# Med = 0.00850, Mean = 0.01167

# Shapiro-Wilk test for normality
shapiro_results_Echinococcus <- all_metadata %>%
  group_by(region) %>%
  summarise(shapiro_p_value = shapiro.test(Echinococcus_.)$p.value)
# Results: p = 0.00000000897 Daneborg, p =  0.00000784 Ittoq
# THEREFORE neither region has normally distributed values

# Wilcoxon Exact rank test
wilcox_exact_results_Echinococcus <- exactRankTests::wilcox.exact(Echinococcus_. ~ region, data = all_metadata)
print(wilcox_exact_results_Echinococcus)
# Results: p =0.312
# THEREFORE no sig difference between regions
...
shapiro_results_Cestoda <- all_metadata %>%
  group_by(region) %>%
  summarise(shapiro_p_value = shapiro.test(Cestoda_.)$p.value)
# Results: p = 3.39e-11 Daneborg, p =  1.03e- 9 Ittoq
# THEREFORE neither region has normally distributed values

# Wilcoxon Exact rank test
wilcox_exact_results_Cestoda <- exactRankTests::wilcox.exact(Cestoda_. ~ region, data = all_metadata)
print(wilcox_exact_results_Cestoda)
# Results: p =0.04305
# THEREFORE YES sig difference between regions
...
summary(all_metadata$Nematoda_.)
# Range   0.02 - 0.4 
# Med = 0.3, Mean = 0.259
...
shapiro_results_Nematoda <- all_metadata %>%
  group_by(region) %>%
  summarise(shapiro_p_value = shapiro.test(Nematoda_.)$p.value)
# Results: p = 0.00000112 Daneborg, p =   0.00000262 Ittoq
# THEREFORE neither region has normally distributed values

# Wilcoxon Exact rank test
wilcox_exact_results_Nematoda <- exactRankTests::wilcox.exact(Nematoda_. ~ region, data = all_metadata)
print(wilcox_exact_results_Nematoda)
# Results: p =0.3481
# THEREFORE no sig difference between regions
...
Viral overall
```{r}
summary(all_metadata$Virus_.)
# Range 0.3-4.0 % reads mapped to Virus
# Med = 0.07%, Mean = 0.225

# Shapiro-Wilk test for normality
shapiro_results_Virus <- all_metadata %>%
  group_by(region) %>%
  summarise(shapiro_p_value = shapiro.test(Virus_.)$p.value)
# Results: p = 0.000237 Daneborg, p = 0.00000000206 Ittoq
# THEREFORE neither region has normally distributed values for % Virus reads

# Wilcoxon Exact rank test
library(exactRankTests)
wilcox_exact_results_Virus <- exactRankTests::wilcox.exact(Virus_. ~ region, data = all_metadata)
print(wilcox_exact_results_Virus)
# Results: p = 0.002321
# THEREFORE YES sig diff in % Virus reads between regions
...
print(wilcox_exact_results_Parvo)
# Results: p = 7.547e-09
# THEREFORE YES sig diff in % Viral reads mapped to Parvo between regions

count_Parvo <- aggregate(PARVO_Parvoviridae ~ region, data = all_metadata, FUN = sum)
# Daneborg = 28, Ittoq = 27
count_Bocaparvovirus <- aggregate(PARVO_Bocaparvovirus ~ region, data = all_metadata, FUN = sum)
# Daneborg = 23, Ittoq = 27
count_Human_Bocavirus <- aggregate(PARVO_Human_Bocavirus ~ region, data = all_metadata, FUN = sum)
# Daneborg = 19, Ittoq = 1
count_Bufavirus_1 <- aggregate(PARVO_Bufavirus_1 ~ region, data = all_metadata, FUN = sum)
# Daneborg 9, Ittoq = 10
count_Canine_Minute <- aggregate(PARVO_Canine_Minute ~ region, data = all_metadata, FUN = sum)
# Daneborg = 0, Ittoq = 1
count_Canine_bufavirus <- aggregate(PARVO_Canine_bufavirus ~ region, data = all_metadata, FUN = sum)
# Daneborg = 0, Ittoq = 1
count_Canine_protoparvovirus <- aggregate(PARVO_Canine_protoparvovirus ~ region, data = all_metadata, FUN = sum)
# Daneborg = 0, Ittoq = 1
count_Canine_parvovirus <- aggregate(PARVO_Canine_parvovirus ~ region, data = all_metadata, FUN = sum)
# Daneborg = 1, Ittoq = 0
...
print(wilcox_exact_results_Adeno)
# Results: p = 0.0001022
# THEREFORE YES sig diff in % Viral reads mapped to Adeno between regions

count_Adeno <- aggregate(ADENO_Adenoviridae ~ region, data = all_metadata, FUN = sum)
# Daneborg = 19 Ittoq = 27
count_Canine_mastadenovirus <- aggregate(ADENO_Canine_mastadenovirus ~ region, data = all_metadata, FUN = sum)
# Daneborg = 0, Ittoq = 3
count_Human_adenovirus <- aggregate(ADENO_Human_adenovirus ~ region, data = all_metadata, FUN = sum)
# Daneborg = 3, Ittoq = 24
...
print(wilcox_exact_results_Pap)
# Results: p = 8.691e-05
# THEREFORE YES sig diff in % Viral reads mapped to Pap between regions

count_Pap <- aggregate(PAP_Papillomaviridae ~ region, data = all_metadata, FUN = sum)
#Daneborg = 28 Ittoq = 29
count_HPV <- aggregate(PAP_HPV ~ region, data = all_metadata, FUN = sum)
# Daneborg = 28, Ittoq = 29
count_Tick_assoc_papillomavirus_lsx <- aggregate(PAP_Tick_assoc_papillomavirus_lsx ~ region, data = all_metadata, FUN = sum)
# Daneborg 0, Ittoq = 25
```
...
```{r}
# Reshape the data for ggplot2 using tidyr
inferred_species_long <- inferred_species %>%
  pivot_longer(cols = -c(sample, region), names_to = "species", values_to = "count")
inferred_species_long <- inferred_species %>%
  pivot_longer(cols = -c(sample, region), names_to = "species", values_to = "count")

# Define a function to group species based on prefixes
group_species <- function(species) {
  if (grepl("^C_", species)) {
    return("C_")
  } else if (grepl("^G_", species)) {
    return("G_")
  } else if (grepl("^Ta_", species)) {
    return("Ta_")
  } else if (grepl("^Tr_", species)) {
    return("Tr_")
  } else if (grepl("^S_", species)) {
    return("S_")
  } else if (grepl("^E_", species)) {
    return("E_")
  } else {
    return("Other")
  }
...
custom_palette <- brewer.pal(n = 7, name = "Set3")

# Add a new column for species group
inferred_species_long <- inferred_species_long %>%
  mutate(species_group = sapply(species, group_species))

# Create a stacked bar graph with facets along the x-axis
ggplot(inferred_species_long, aes(x = sample, y = count, fill = species_group)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = setNames(custom_palette, unique(inferred_species_long$species_group))) +  # Use custom color palette
  facet_wrap(~region, scales = "free_x") +  # Display scales along the bottom
...
# DA of potentially pathogenic bacteria:
```{r}
pathogenic_physeq = subset_taxa(physeq_clr_genus, genus=="Bacteroides" | genus== "Campylobacter_D" | genus== "Clostridium"| genus== "Clostridium_AH"| genus== "Clostridium_G"| genus== "Clostridium_H"| genus== "Clostridium_I"| genus== "Clostridium_Q"| genus== "Enterococcus_B"| genus== "Escherichia"| genus== "Fusobacterium_A"| genus== "Fusobacterium_B" | genus== "Helicobacter_A" | genus== "Helicobacter_B" | genus== "Helicobacter_C" | genus== "Helicobacter_D" | genus== "Helicobacter_G" | genus== "Klebsiella" | genus== "Streptococcus")

pathogenic_physeq.t <- as.data.frame(t(as.matrix(pathogenic_physeq@otu_table)))
table.W_pathogenic <- tibble::rownames_to_column(pathogenic_physeq.t, "sample")
sample_type <- metadata[,c(1,4)]
taxa_table_pathogenic <- merge(table.W_pathogenic,sample_type,by="sample")
taxa_table_pathogenic <- taxa_table_pathogenic[,-c(1)]
taxa_table_pathogenic.no <- taxa_table_pathogenic[,-ncol(taxa_table_pathogenic)]
all.taxa_pathogenic <- colnames(taxa_table_pathogenic.no)
Wilcox_result_pathogenic <- c()
for (y in all.taxa_pathogenic){
  res.wilcox_pathogenic <- wilcox.test(taxa_table_pathogenic[,y] ~ region, data = taxa_table_pathogenic,
                            exact = FALSE, alternative = "less")
  Wilcox_result_pathogenic <- rbind(Wilcox_result_pathogenic,c(res.wilcox_pathogenic[[1]],Pvalue=res.wilcox_pathogenic[[3]]))
}
rownames(Wilcox_result_pathogenic) <- all.taxa_pathogenic
Wilcox_result_pathogenic <- as.data.frame(Wilcox_result_pathogenic)

Wilcox_result_pathogenic$p_adjusted <- p.adjust(Wilcox_result_pathogenic$Pvalue, method = "fdr")

Wilcox_result_sign_pathogenic <- subset(Wilcox_result_pathogenic, p_adjusted <= 0.05) %>%
  rownames_to_column(., "Phylum")
knitr::kable(Wilcox_result_pathogenic, format = "html", full_width = F,col.names = c("Genus", "W", "p-value","p-value adjusted"), digits = 10) %>%
  kable_styling(latex_options="scale_down")

knitr::kable(Wilcox_result_sign_pathogenic, format = "html", full_width = F,col.names = c("Genus", "W", "p-value","p-value adjusted"), digits = 10) %>%
  kable_styling(latex_options="scale_down")

# No sig differences in potentially pathogenic bacteria when samples are grouped by region
```