...
beta_div_neutral <- count_filtered %>%
    hillpair(., q = 1) #calculate neutral beta diversities and distance matrix
```
...
# Permutation test
# ps.disper.neutral <- betadisper(hill_pair_dis_dist, all_metadata$region, type = c("median"), bias.adjust = FALSE, sqrt.dist = FALSE, add = FALSE)

# Error in pts[groups == i, , drop = FALSE] : (subscript) logical subscript too long

# permutest(ps.disper.neutral, pairwise = TRUE, permutations = 999) # cannot run due to error in ps.disper.neutral 

# a p value of >0.05 in this case will tell you that the data is in an ok format for running an adonis
## results: 

meta_order <- all_metadata[order(all_metadata$sample),] 
m1 <- as.matrix(hill_pair_dis_dist) 
metadata_ord<- all_metadata[order(match(all_metadata$sample,rownames(m1))),] 
# this section correctly orders the metadata so that it matches to the beta diversity distance matrix

# Run test
# adonis2(hill_pair_dis_dist ~ region, data =metadata_ord, permutations = 999)
