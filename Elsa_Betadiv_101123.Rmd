...
#PROBLEM: EHI01293 is missing from hill_pair_dis_dist, and hill_pair_dis_nmds, but IS present in hill_pair_dis 
hill_pair_dis_selected <- hill_pair_dis %>% 
  select(first, second, C)
#EHI01293 is the first 57 entries in "first" and never appears in "second"

hill_pair_dis_df <- as.data.frame(hill_pair_dis_selected) 
#EHI01293 is the first 57 entries in "first"

hill_pair_dis_pivoted <- pivot_wider(hill_pair_dis_df, names_from = first, values_from = C)
# EHI01293 is the first column name, but is not in "second"

hill_pair_dis_rownames <- column_to_rownames(hill_pair_dis_pivoted, var = "second")
# in between these two steps EHI01293 goes missing. EHI01293 is the first column name of hill_pair_dis_rowname, but then is not included as a row or column name for hill_pair_dis_dist_matrix
hill_pair_dis_dist_matrix <- as.matrix(hill_pair_dis_dist)

hill_pair_dist_dist <- as.dist(hill_pair_dis_dist_matrix)

ps.disper.neutral <- betadisper(hill_pair_dis_dist, all_metadata$region)
