...
```{r}
library(DESeq2)
deseq1<- phyloseq_to_deseq2(physeq, ~ region) # could not get to run with physeq_clr
deseq1 <- DESeq(deseq1)
results_table <- results(deseq1)
```
```{r}
results_table$padj <- p.adjust(results_table$pvalue, method = "BH")
alpha <- 0.05
sig_results <- subset(results_table, padj < alpha)
```
```{r}
plotMA(deseq1)
plotDispEsts(deseq1, main = "region")
```
...
lfc_threshold <- 0  # Log-fold change threshold (0 for upregulated genes, adjust as needed)

expressedUP_genes <- subset(results_table, padj < alpha & log2FoldChange > lfc_threshold)
expressedDOWN_genes <- subset(results_table, padj < alpha & log2FoldChange < -lfc_threshold)
```

for expressedUP: Genes with positive log2FoldChange values are positively enriched in "Ittoqqortoormii" compared to "Daneborg."

for expressedDOWN: Genes with negative log2FoldChange values are negatively enriched in "Ittoqqortoormii" compared to "Daneborg."

```{r}
# Heatmap showing top diff expressed genes

library(pheatmap)
top_genes <- rownames(sig_results)[sig_results$padj < 0.1]
pheatmap(assay(deseq1)[top_genes, ],scale = "row", main = "Top Differentially Expressed Genes")
```
