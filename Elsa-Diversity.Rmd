...
```
```{r}
   # adonis2(beta_div_func_S ~ ., data=sample_table_adonis_func[labels(beta_div_func_S),], permutations=999) %>%
  #        as.matrix() %>%
   #         kable()
```
...
```
```{r}
group_f <- length(unique(beta_div_func_S_nmds$region))
...
# try the EHI workflow and old code to make sure the adonis results are the same
